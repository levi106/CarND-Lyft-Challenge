...
@test_safe
def test_for_lyft_dataset(data_dir):
    lyft_dataset_path = os.path.join(data_dir, '.')
    training_labels_count = len(glob(os.path.join(lyft_dataset_path, 'Train/CameraSeg/*.png')))
    training_images_count = len(glob(os.path.join(lyft_dataset_path, 'Train/CameraRGB/*.png')))
    testing_images_count = len(glob(os.path.join(lyft_dataset_path, 'Test/CameraRGB/*.png')))

    assert not (training_images_count == training_labels_count == testing_images_count == 0), \
        'Lyft dataset not found. Extract Lyft dataset in {}'.format(lyft_dataset_path)
    assert training_images_count == 950, 'Expected 950 training images, found {} images.'.format(training_images_count)
    assert training_labels_count == 950, 'Expected 950 training labels, found {} labels.'.format(training_labels_count)
    assert testing_images_count == 50, 'Expected 50 testing images, found {} images.'.format(testing_images_count)
